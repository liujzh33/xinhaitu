...
from .attention_mask import flash_attention
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs,
                ...
                vlm_q: torch.Tensor = None,
                vlm_k: torch.Tensor = None,
                vlm_v: torch.Tensor = None,
        ...
        Args:
            ...
            vlm_q/k/v(Tensor, optional): VLM Q/K/V for trimodal MoT (NEW)
            attn_mask(Tensor, optional): Custom attention mask [B, Lq, Lk], True=allow, False=mask
            und_q/k/v(Tensor, optional): Understanding expert Q/K/V for trimodal MoT (legacy)
        """
        ...
        # Trimodal MoT branch: WAN + Action + VLM (or Understanding for legacy)
        # Support both vlm_q/k/v (new) and und_q/k/v (legacy) for backward compatibility
        if action_q is not None or vlm_q is not None or und_q is not None:
            L_x = q.size(1)
            ...
            # Add VLM tokens if provided (NEW)
            # Fall back to und_q/k/v for backward compatibility
            if vlm_q is not None:
                q_parts.append(vlm_q)
                k_parts.append(vlm_k)
                v_parts.append(vlm_v)
                L_vlm = vlm_q.size(1)
                use_vlm = True
            elif und_q is not None:
            ...
            if L_vlm > 0:
                vlm_or_und_out = attn_out[:, start_idx:start_idx+L_vlm, :, :]
                outputs.append(vlm_or_und_out)
            else:
                outputs.append(None)
