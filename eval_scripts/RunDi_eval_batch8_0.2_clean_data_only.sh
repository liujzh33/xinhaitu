#!/bin/bash
# Eval script for: batch8_0.2_clean_data_only
# Checkpoints: only 1 checkpoint (pytorch_model at root level, still training)
# Test order: latest checkpoint only
...
# ============================================================================
# Define checkpoint to test
# NOTE: clean_data_only paths have pytorch_model at root level (no step subdirs)
# ============================================================================
CKPT_OBS_BASE="obs://yw-2030-gy/external/wwx1513998/checkpoint/motus_robotwin_subtask_Motus_threeclip_20w_batch8_0.2_clean_data_only"
RUN_TAG="batch8_0.2_clean_data_only"

# Check for sub-directories (checkpoint_step_*, best_action_l2) first
# Then fall back to root-level pytorch_model
CKPT_LIST=()

# Check for best_action_l2
conda activate qwen3
if python -c "import moxing as mox; exit(0 if mox.file.is_directory('${CKPT_OBS_BASE}/best_action_l2/pytorch_model') else 1)"; then
    CKPT_LIST+=("best_action_l2")
fi

# Check for step checkpoints (descending order)
for step in 200000 180000 160000 140000 120000 100000 80000 60000 40000 20000; do
    if python -c "import moxing as mox; exit(0 if mox.file.is_directory('${CKPT_OBS_BASE}/checkpoint_step_${step}/pytorch_model') else 1)"; then
        CKPT_LIST+=("checkpoint_step_${step}")
    fi
done

# If no sub-directories found, use root-level pytorch_model
if [ ${#CKPT_LIST[@]} -eq 0 ]; then
    echo "No step sub-directories found, using root-level pytorch_model"
    CKPT_LIST+=("__root__")
fi

echo "========================================================="
echo "Testing ${#CKPT_LIST[@]} checkpoint(s) for $RUN_TAG"
...
for CKPT_NAME in "${CKPT_LIST[@]}"; do
    ...
    # Determine OBS and local paths
    if [ "$CKPT_NAME" = "__root__" ]; then
        CKPT_OBS="${CKPT_OBS_BASE}/pytorch_model"
        CKPT_LOCAL="/cache/wx1513998/eval_checkpoints/${RUN_TAG}/root/pytorch_model"
    else
...
"
    ...
    rm -rf "/cache/wx1513998/eval_checkpoints/${RUN_TAG}"
...
echo ""
echo "========================================================="
echo "All ${#CKPT_LIST[@]} checkpoint(s) for $RUN_TAG completed!"
echo "========================================================="
