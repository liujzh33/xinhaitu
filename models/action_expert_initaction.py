...
class ActionExpert(nn.Module):
    ...
    # Prefixes of modules that skip loading from WAN (Action-specific)
    ACTION_BACKBONE_SKIP_PREFIXES = (
        "input_encoder.",   # state/action encoder
        "decoder.",         # action head
        "registers",        # register tokens
        "wan_action_",      # WAN projection modules
    )

    # Meta keys that must match between WAN and Action configs
    ...
    def backbone_key_set(cls, keys) -> set[str]:
        """Return set of keys to load from WAN pre-trained weights."""
        backbone_keys = set()
        for key in keys:
            # Skip keys that contain skip_prefixes anywhere in the path
            skip = False
            for prefix in cls.ACTION_BACKBONE_SKIP_PREFIXES:
                if prefix in key:
                    skip = True
                    break
            if not skip:
    ...
    def from_pretrained(
        ...
        action_pretrained_path: str,
        wan_config: dict,
        device: str = "cuda",
        torch_dtype: torch.dtype = torch.bfloat16,
    ) -> "ActionExpert":
        """
        Load Action Expert from pre-processed WAN weights.

        Args:
            config: Action Expert configuration
            action_pretrained_path: Path to pre-processed .pt file
            wan_config: WAN configuration
            device: Device to load model on
            torch_dtype: Data type for model weights

        Returns:
            ActionExpert instance with pre-trained backbone loaded
        """
        # Create model
        action_expert = cls(config, wan_config).to(device=device, dtype=torch_dtype)
        action_state = action_expert.state_dict()
        expected_backbone_keys = cls.backbone_key_set(action_state.keys())

        # Load pre-trained weights
        payload = torch.load(action_pretrained_path, map_location="cpu")

        # Validate meta information
        meta = payload.get("meta")
        if meta is None:
            raise ValueError(f"`meta` field missing in {action_pretrained_path}")
        for key in cls.ACTION_BACKBONE_META_KEYS:
            if key not in meta:
                raise ValueError(f"`meta.{key}` missing in {action_pretrained_path}")

        # Load backbone weights
        backbone_state_dict = payload["backbone_state_dict"]
        if backbone_state_dict is None:
            raise ValueError(f"`backbone_state_dict` missing in {action_pretrained_path}")
        ...
        if missing_keys or unexpected_keys:
            logger.warning(
                f"Key mismatch in {action_pretrained_path}: "
                f"missing={missing_keys[:10]}..., unexpected={unexpected_keys[:10]}..."
            )

        # Copy backbone weights to action state
        for key in expected_backbone_keys:
            if key in backbone_state_dict:
                value = backbone_state_dict[key]
                if tuple(value.shape) != tuple(action_state[key].shape):
                    logger.warning(
                        f"Shape mismatch for `{key}` in {action_pretrained_path}: "
                        f"expected {tuple(action_state[key].shape)}, got {tuple(value.shape)}"
                    )
                action_state[key] = value.to(device=device, dtype=torch_dtype)

        # Apply state dict
        action_expert.load_state_dict(action_state, strict=False)

        logger.info(f"Loaded ActionExpert backbone from {action_pretrained_path}")
        logger.info(f"  Loaded {len(expected_backbone_keys)} backbone keys")
        logger.info(f"  Skipped {len(action_state) - len(expected_backbone_keys)} non-backbone keys")
