...
class MotusConfig:
    ...
    # WAN frozen setting
    wan_frozen: bool = True   # WAN is frozen by default for this version
...
class UndModule(nn.Module):
    ...
    def extract_und_features(
        ...
        adapter_time = time.time() - t_adapter_start

        # Log timing breakdown (every step for debugging)
        # logger.info(f"UndModule extract breakdown:")
        # logger.info(f"  Process inputs: {process_time*1000:.1f}ms")
        # logger.info(f"  VLM forward: {vlm_time*1000:.1f}ms")
        # logger.info(f"  VLM adapter: {adapter_time*1000:.1f}ms")
    ...
    def _process_vlm_inputs_to_tokens(self, vlm_inputs, B: int) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor], Optional[list], torch.Tensor]:
        ...
        text_embed_time = time.time() - t_text_start
        ...
        image_embed_time = time.time() - t_image_start

        t_image_cat_start = time.time()
        image_embeds = torch.cat(image_embeds, dim=0).to(self.device, self.dtype)
        image_cat_time = time.time() - t_image_cat_start
        ...
        pos_time = time.time() - t_pos_start

        # Log detailed breakdown (every step for debugging)
        # logger.info(f"UndModule process_inputs breakdown:")
        # logger.info(f"  Text embeddings: {text_embed_time*1000:.1f}ms")
        # logger.info(f"  Image features: {image_embed_time*1000:.1f}ms")
        # logger.info(f"  Image cat/convert: {image_cat_time*1000:.1f}ms")
        # logger.info(f"  Insert images: {insert_time*1000:.1f}ms")
        # logger.info(f"  Position ids: {pos_time*1000:.1f}ms")
...
class Motus(nn.Module):
    ...
    def __init__(self, config: MotusConfig):
        ...
        # Freeze WAN parameters if specified
        if config.wan_frozen:
            for param in self.video_model.wan_model.parameters():
                param.requires_grad = False
            logger.info("WAN backbone parameters frozen")
        else:
            logger.info("WAN backbone parameters are TRAINABLE")
    ...
    def log_parameter_counts(self):
        ...
        video_trainable_params = sum(p.numel() for p in self.video_model.parameters() if p.requires_grad)
        ...
        logger.info(f"  Video Model (WAN): {video_params / 1e9:.2f}B (trainable: {video_trainable_params / 1e9:.2f}B)")
