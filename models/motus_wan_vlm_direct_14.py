...
@dataclass
class MotusWanVlmDirectConfig:
    """Configuration for MotusWanVlmDirect."""
    ...
    # Understanding Expert settings for layers 14-29 (16 blocks total)
    und_expert_enabled: bool = True              # Enable Understanding Expert
    ...
    # MoT layer participation
    vlm_mot_layers: int = 14                     # VLM participates in layers 0-13
    und_mot_start_layer: int = 14                # Understanding Expert starts at layer 14
...
class VideoModule(nn.Module):
    ...
    def process_und_ffn(
        ...
        ffn_output = und_block.ffn(ffn_input)
    ...
    def process_joint_attention_with_und(
        self,
        ...
        """Trimodal joint self-attention: WAN + Action + Understanding Expert via WAN self-attn (MoT)."""
        wan_layer = self.video_model.wan_model.blocks[layer_idx]
        ...
        # Pre-attn normalization with AdaLN (video/action) or regular norm (und)
        ...
        L_und = norm_und.shape[1]

        # Und heads for WAN space (512 -> 24*128)
        u_qkv = torch.einsum("BTD,KNDE->KBTNE", norm_und, und_block.wan_und_qkv)
        u_q_h, u_k_h, u_v_h = u_qkv[0], u_qkv[1], u_qkv[2]
        u_q = und_block.wan_und_norm_q(u_q_h.flatten(-2)).view(B, L_und, n, d)
        u_k = und_block.wan_und_norm_k(u_k_h.flatten(-2)).view(B, L_und, n, d)
        u_v = u_v_h.view(B, L_und, n, d)

        # Meta info for WAN attention
        seq_lens = torch.full((B,), L_v + L_a + L_und, dtype=torch.long, device=self.device)
        freqs = self.video_model.wan_model.freqs
        if freqs.device != self.device:
            freqs = freqs.to(self.device)

        # Call WAN self-attn with trimodal MoT (using und_q/k/v)
        y, action_out_h, und_out_h = wan_layer.self_attn(
            ...
            und_q=u_q, und_k=u_k, und_v=u_v  # Use und_q/k/v
        )

        # Project Und output back to Und dimension (3072 -> 512)
    ...
    def process_joint_attention(
        ...
        # Call WAN self-attn with trimodal MoT (using vlm_q/k/v instead of und_q/k/v)
        y, action_out_h, vlm_out_h = wan_layer.self_attn(
            norm_video, seq_lens, self.grid_sizes, freqs,
            action_q=a_q, action_k=a_k, action_v=a_v,
            vlm_q=v_q, vlm_k=v_k, vlm_v=v_v  # Use vlm_q/k/v (NEW PARAMETER)
        )
...
class MotusWanVlmDirect(nn.Module):
    """
    Modular Three-modal UniDiffuser with VGM (WAN), VLM (Direct MoT), and Action modules.

    Key differences from original Motus:
    - VLM participates directly in per-layer joint attention (no Understanding Expert)
    - VLM has 28 layers, WAN has 30 layers (VLM Layer 26 mapped to WAN Layer 28, VLM Layer 27 to WAN Layer 29)
    - VLM is trainable (not frozen)
    """

    def __init__(self, config: MotusWanVlmDirectConfig):
        super().__init__()
        self.config = config
        ...
        # Create config dictionaries for ActionExpert and UnderstandingExpert
        wan_config = {
            'dim': wan_dim,
        ...
        vlm_config = {
            'num_heads': vlm_num_heads,
            'num_kv_heads': vlm_num_kv_heads,
        ...
        # Initialize Understanding Expert (auto-calculated layer count)
        if self.config.und_expert_enabled:
            und_num_layers = config.num_layers - config.vlm_mot_layers
            und_start = config.vlm_mot_layers
            und_end = config.num_layers - 1
            logger.info(f"Initializing {und_num_layers}-layer Understanding Expert for layers {und_start}-{und_end}...")
            und_config = UndExpertConfig(
                dim=self.config.und_expert_hidden_size,
                ffn_dim=self.config.und_expert_hidden_size * self.config.und_expert_ffn_dim_multiplier,
                num_layers=und_num_layers,  # Auto-calculated from vlm_mot_layers
                vlm_input_dim=self.config.vlm_adapter_input_dim,
                vlm_projector_type=self.config.vlm_adapter_projector_type,
                eps=self.config.und_expert_norm_eps,
            )
    ...
    def training_step(
        ...
        with torch.autocast(device_type="cuda", dtype=self.video_model.precision):
            ...
            # VLM participation limit
            vlm_mot_cutoff = self.config.vlm_mot_layers
            und_mot_start = self.config.und_mot_start_layer

            # Initialize und_tokens (will be initialized at layer 14 from VLM)
            und_tokens = None

            for layer_idx in range(self.config.num_layers):
                # VLM tokens for this layer: only for layers 0-13
                if layer_idx < vlm_mot_cutoff:
                ...
                # Initialize und_tokens at layer 14 with VLM layer 14 output
                if layer_idx == und_mot_start and self.und_expert is not None:
                    vlm_layer_14_output = vlm_per_layer[layer_idx]  # VLM layer 14 output
                    # Pass through VLM adapter to get und_tokens
                    und_tokens = self.und_expert.vlm_adapter(vlm_layer_14_output)
                ...
                # Joint attention based on layer index
                t_joint_attn = time.time()
                if layer_idx < vlm_mot_cutoff:
                    # Trimodal: WAN + Action + VLM
                ...
                elif self.und_expert is not None:
                    # Trimodal: WAN + Action + Understanding Expert
                    # Map WAN layer idx to undExpert block idx (WAN 14-29 → Und 0-15)
                    und_block_idx = layer_idx - und_mot_start
                    video_tokens, action_tokens, und_tokens = self.video_module.process_joint_attention_with_und(
                        video_tokens, action_tokens, und_tokens,
                        video_adaln_modulation, action_adaln_modulation, layer_idx,
                        self.action_expert.blocks[layer_idx],
                        self.und_expert.blocks[und_block_idx]
                    )
                    joint_attn_time = time.time() - t_joint_attn
                    ...
                    wan_attn_time += joint_attn_time / 3
                    action_attn_time += joint_attn_time / 3
                    und_attn_time += joint_attn_time / 3

                    # Understanding Expert FFN
                    t_ffn_und = time.time()
                    und_tokens = self.video_module.process_und_ffn(und_tokens, self.und_expert.blocks[und_block_idx])
                    und_ffn_time += time.time() - t_ffn_und
        ...
        self._timing['und_attn'] = und_attn_time
        self._timing['und_ffn'] = und_ffn_time
    ...
    def inference_step(
        ...
        for i in range(num_inference_steps):
            ...
            # Trimodal MoT forward - joint denoising for WAN, Action, VLM/Und
            with torch.autocast(device_type="cuda", dtype=self.video_model.precision):
                # Time embeddings
