...
# 对，现在“labels 监督文本”和真实 subtask 基本已经对上了。

# 从日志看，debug 里已经能看到完整的 supervised target：

# ```text
# supervised target text: Shift the shoe into position over the mat.
# supervised target text: Close the fingers around the shoes.
# supervised target text: Approach the movable object and prepare to place it on the weighing platform.
# supervised target text: Guide the can to the basket interior.
# ```

# 这些都没有漏掉第一个动词，说明之前 `Move / Bring / Close / Guide` 被 mask 掉的问题已经修好了。

# 但现在还有一个新现象：

# ```text
# [Subtask] target: Position the gripper near the laptop lid.
# [Subtask] pred: the the the the the the the the the
# ```

# 这个不是 labels 对不上的问题，而是模型还在训练最早期。你这里才是 Step 20，而且 `subtask_loss` 还很高：

# ```text
# Step 20 ... Subtask: 8.0967
# ```

# 所以 Qwen3-VL 还没有学会当前数据格式下的 subtask 输出，出现 `the the the...` 这种重复是正常的早期语言模型退化现象。

# 可以这样判断：

# ```text
# labels / target 构造：已经基本对了；
# 模型预测质量：还没训练好；
# 目前 pred 重复：正常早期现象，不是 mask 错位问题。
# ```

# 另外：

# ```text
# input text (first 300 chars): <|image_pad|><|image_pad|>...
# ```

# 这个也正常。因为 Qwen-VL 输入前面有大量 image placeholder token，前 300 个字符全是 `<|image_pad|>` 不代表文本没进去，只是图像 token 太多，把文本挤到后面了。

# 建议你继续观察到 Step 500 / 1000 / 2000：

# ```text
# 如果 target 一直完整，subtask_loss 持续下降，pred 从重复词逐渐变成接近 target，就说明修复成功。
# ```

# 现在最重要的验收点已经通过了：`supervised target text` 已经包含完整第一个动词。
...
class MotusWanVlmDirectMask(nn.Module):
    ...
    def __init__(self, config: MotusWanVlmDirectMaskConfig):
        ...
        # Subtask prediction: reuse VLM lm_head for token-level CE loss
        subtask_cfg = getattr(config, 'subtask_prediction', None) or {}
        self.subtask_prediction_enabled = subtask_cfg.get('enabled', False)
        self.subtask_loss_weight = subtask_cfg.get('loss_weight', 1.0)

        if self.subtask_prediction_enabled:
            # Use Qwen3-VL's native LM for subtask text generation (Pi0.5-style)
            # No separate head needed — the VLM's own transformer + lm_head does the work.
            # subtask_lm_inputs contains image + "Task: ... Subtask: {gt_text}" with labels.
            # The VLM forward with labels computes CE loss only on subtask tokens.
            self.vlm_vocab_size = self.vlm_model.config.text_config.vocab_size
            logger.info(f"Subtask prediction ENABLED via Qwen3-VL native LM (loss_weight={self.subtask_loss_weight}, vocab_size={self.vlm_vocab_size})")
        else:
    ...
    def training_step(
        ...
        subtask_lm_inputs: Optional[Dict[str, torch.Tensor]] = None,  # VLM inputs with labels for native LM loss
        ...
        # ===== Subtask prediction loss (Qwen3-VL native LM) =====
        subtask_loss = torch.tensor(0.0, device=self.device, requires_grad=True)
        decoded_subtask_pred = ""
        decoded_subtask_target = ""
        if self.subtask_prediction_enabled and subtask_lm_inputs is not None:
        ...
        if self.progress_detection_enabled and progress_targets is not None:
            ...
            action_pooled = action_pooled.detach().to(self.dtype)  # Detach! Prevent progress loss from corrupting action expert
    ...
    def inference_step(
        ...
        # Subtask prediction during inference using Qwen3-VL generate
        predicted_subtask = None
        if self.subtask_prediction_enabled and vlm_inputs is not None:
            with torch.no_grad():
