...
class VideoModule(nn.Module):
    ...
    def process_joint_attention(
        ...
        # Attention pattern: Action看所有, Video看Video+Action, VLM看VLM+Action, Video与VLM互相不可见
        L_total = L_v + L_a + L_vlm
        attn_mask = torch.ones((B, L_total, L_total), dtype=torch.bool, device=self.device)

        # Video 能看 Video + Action，不能看 VLM
        attn_mask[:, :L_v, L_v + L_a:] = False

        # VLM 能看 VLM + Action，不能看 Video
        attn_mask[:, L_v + L_a:, :L_v] = False

        # Action 能看所有（默认为True，无需修改）
        # =================================================================
        ...
        if layer_idx == 0 and not hasattr(self, '_mask_printed'):
            ...
            logger.info("V=Video, A=Action, L=VLM")
            logger.info("Expected: Video→VA✅, Action→VAL✅, VLM→AL✅, V↔L:❌")
            logger.info("=" * 60)
