...
from .action_expert_fast import ActionExpert, ActionExpertConfig
...
class MotusWanVlmDirectMaskConfig:
    ...
    # Action Expert pretrained backbone path
...
class VideoModule(nn.Module):
    ...
    def process_joint_attention(
        ...
        # ==================== FastWAM first_frame_causal mask ====================
        # Attention pattern:
        #   - Video 第一帧不看后续帧（条件帧）
        #   - Video 后续帧可以互相看见
        #   - Action 只看 Video 第一帧
        #   - Action 看 Action 自注意力
        #   - Action 看 VLM
        #   - VLM 只看 VLM 自注意力
        L_total = L_v + L_a + L_vlm
        attn_mask = torch.ones((B, L_total, L_total), dtype=torch.bool, device=self.device)

        # 计算每个 video frame 的 token 数
        # grid_sizes: [B, 3] -> [T, H, W]
        lat_T, lat_H, lat_W = self.grid_sizes[0].cpu().tolist()
        tokens_per_frame = lat_H * lat_W  # 例如: 12*10=120 for 384x320 input

        # 1. Video 内部: first_frame_causal
        #    第一帧不看后续帧（条件帧）
        #    后续帧可以互相看见
        first_frame_tokens = min(tokens_per_frame, L_v)
        attn_mask[:, :first_frame_tokens, first_frame_tokens:L_v] = False

        # 2. Video 不看 Action 和 VLM
        attn_mask[:, :L_v, L_v:] = False

        # 3. Action 只看 Video 第一帧（条件帧）
        #    Action → Video 第一帧: ✅
        #    Action → Video 后续帧: ❌
        attn_mask[:, L_v:L_v+L_a, first_frame_tokens:L_v] = False
        ...
        if layer_idx == 0 and not hasattr(self, '_mask_printed'):
            logger.info("=" * 60)
            logger.info(f"FastWAM first_frame_causal mask:")
            logger.info(f"  Grid: T={lat_T}, H={lat_H}, W={lat_W}, tokens_per_frame={tokens_per_frame}")
            logger.info(f"  L_v={L_v}, L_a={L_a}, L_vlm={L_vlm}")
            logger.info("Rows=Query, Cols=Key, 1=allow, 0=mask")
            # Print compact representation
            mask_np = attn_mask[0].cpu().numpy().astype(int)
            logger.info(f"Video frame0 rows: {mask_np[:tokens_per_frame, :2*L_v]}")
            logger.info(f"Video frame1+ rows: {mask_np[tokens_per_frame:min(2*tokens_per_frame, L_v), :2*L_v]}")
            logger.info(f"Action rows: {mask_np[L_v:L_v+1,:L_v]}")
            logger.info(f"  Expected: V0→V0✅,V0→V1+❌,V1+→V0+✅, A→V0✅,A→V1+❌")
            logger.info("=" * 60)
...
class MotusWanVlmDirectMask(nn.Module):
    ...
    def __init__(self, config: MotusWanVlmDirectMaskConfig):
        ...
        # Initialize Action Expert - load from pre-trained backbone if path is provided
    ...
    def _prefill_video_kv_cache(
        ...
        """
        Precompute Video K/V cache for fast action-only inference.
        Runs full video forward (attention + cross_attn + FFN) per layer to get
        correct per-layer K/V, same as FastWAM's prefill_video_cache.

        Args:
            video_tokens: Video tokens [B, L_v, C]
            video_adaln_params: Video AdaLN modulation parameters
            processed_t5_context: Pre-processed T5 context for cross attention
            grid_sizes: Grid sizes for RoPE
            freqs: RoPE frequencies

        Returns:
            video_kv_cache: List of {k, v} dicts per layer
        """
        ...
        for layer_idx in range(self.config.num_layers):
            ...
            # Video normalization (AdaLN modulation)
            v_mod = self.video_module.compute_adaln_modulation(video_adaln_params, layer_idx)
            norm_video = wan_layer.norm1(video_tokens).float() * (1 + v_mod[1].squeeze(2)) + v_mod[0].squeeze(2)

            L_v = video_tokens.shape[1]

            with torch.amp.autocast('cuda', dtype=self.dtype):
                # Video Q/K/V
                video_k = wan_layer.self_attn.norm_k(wan_layer.self_attn.k(norm_video)).view(B, L_v, n, d)
                video_v = wan_layer.self_attn.v(norm_video).view(B, L_v, n, d)
                video_q = wan_layer.self_attn.norm_q(wan_layer.self_attn.q(norm_video)).view(B, L_v, n, d)

            # Apply RoPE to Video K and Q (outside autocast, needs float)
            video_k_rope = rope_apply(video_k, grid_sizes, freqs)
            video_q_rope = rope_apply(video_q, grid_sizes, freqs)

            # Cache this layer's Q/K/V
            video_kv_cache.append({"k": video_k_rope, "v": video_v, "q": video_q_rope})

            # === Update video_tokens for next layer (same as FastWAM) ===
            # Video self-attention (only 1 frame, no causal mask needed)
            seq_lens = torch.full((B,), L_v, dtype=torch.long, device=self.device)
            from wan.modules.attention_mask import flash_attention
            video_out = flash_attention(
                q=video_q_rope,
                k=video_k_rope,
                v=video_v,
                q_lens=None,
            ...
            with torch.amp.autocast('cuda', dtype=self.dtype):
                video_tokens = video_tokens + wan_layer.self_attn.o(video_out.flatten(2)) * v_mod[2].squeeze(2)
            ...
            # Video FFN
            video_tokens = self.video_module.process_ffn(video_tokens, v_mod, layer_idx)

        return video_kv_cache

    @torch.no_grad()
    def infer_action_only(
        self,
        first_frame: torch.Tensor,
        state: torch.Tensor = None,
        ...
        """
        Action-only inference (FastWAM-style).

        Only inputs a single first_frame and predicts actions, no video generation.

        Args:
            first_frame: Initial frame [B, C, H, W] or [C, H, W]
            state: Initial robot state [B, state_dim]
        ...
        # Normalize first_frame to [-1, 1]
        first_frame_norm = (first_frame * 2.0 - 1.0).unsqueeze(2)  # [0,1] -> [-1,1], [B, C, 1, H, W]

        # Encode to video latent (only 1 frame)
        condition_frame_latent = self.video_model.encode_video(first_frame_norm.to(self.dtype))  # [B, C', 1, H', W']

        # Init action latent
        # Use the model's configured action_chunk_size
        action_chunk_size = self.config.action_chunk_size
        action_latent = torch.randn((B, action_chunk_size, self.config.action_dim), device=self.device, dtype=self.dtype)

        # Preprocess T5 context
        processed_t5_context = self.video_module.preprocess_t5_embeddings(language_embeddings)

        # Prepare video tokens (condition frame only)
        video_latent = condition_frame_latent  # [B, C', 1, H', W']
        video_tokens = self.video_module.prepare_input(video_latent.to(self.dtype))

        # Precompute video info (cache for efficiency, similar to FastWAM)
        video_t_scaled = torch.zeros(B, device=self.device, dtype=self.dtype)  # t=0 for condition frame
        video_head_time_emb, video_adaln_params = self.video_module.get_time_embedding(video_t_scaled, video_tokens.shape[1])
        ...
        vlm_tokens = vlm_per_layer[0]

        # Get dimensions for attention computation
        n = self.video_model.wan_model.num_heads
        d = 3072 // n
        L_v = video_tokens.shape[1]
        L_vlm = vlm_tokens.shape[1]

        # Get grid sizes and tokens_per_frame
        lat_T, lat_H, lat_W = self.grid_sizes[0].cpu().tolist()
        tokens_per_frame = lat_H * lat_W
        first_frame_tokens = min(tokens_per_frame, L_v)

        # For single-frame inference, adjust grid_sizes to match actual video_tokens length
        # grid_sizes format: [F, H, W] where F=number of frames
        if L_v == tokens_per_frame:  # Single frame
            grid_sizes_single = torch.tensor([[1, lat_H, lat_W]], device=self.device, dtype=self.grid_sizes.dtype).expand(B, -1)
        else:
            # Multiple frames, use original grid_sizes
            grid_sizes_single = self.grid_sizes[:B]

        # === Precompute Video K/V cache (FastWAM-style, per-layer correct K/V) ===
        freqs = self.video_model.wan_model.freqs
        if freqs.device != self.device:
            freqs = freqs.to(self.device)

        video_kv_cache = self._prefill_video_kv_cache(
            video_tokens, video_adaln_params, processed_t5_context, grid_sizes_single, freqs
        )

        # Import flash_attention with attn_mask support
        from wan.modules.attention_mask import flash_attention
        from wan.modules.model import rope_apply
        ...
        for i in range(num_inference_steps):
            ...
            # Prepare action tokens with registers (same as inference_step)
            if self.config.training_mode == 'pretrain':
                # pretrain: no state token
                if self.action_expert.registers is not None:
                    ...
                    action_tokens = self.action_expert.input_encoder(None, action_latent, None)
            else:
                # finetune: has state token and registers
                state_tokens = state.unsqueeze(1).to(self.dtype)
                if self.action_expert.registers is not None:
                    registers = self.action_expert.registers.expand(B, -1, -1)
                    action_tokens = self.action_expert.input_encoder(state_tokens, action_latent, registers)
                else:
                    action_tokens = self.action_expert.input_encoder(state_tokens, action_latent, None)

            action_head_time_emb, action_adaln_params = self.action_module.get_time_embedding(action_t_scaled, action_tokens.shape[1])

            L_a = action_tokens.shape[1]

            # === Reset VLM tokens for each denoising step ===
            # Each denoising step (flow step) should start fresh vlm_tokens,
            # consistent with training where each training sample is independent.
            vlm_tokens = vlm_per_layer[0].clone()

            # Trimodal MoT forward with VLM tokens updating layer-by-layer
            # Same as process_joint_attention but using cached Video K/V
            with torch.autocast(device_type="cuda", dtype=self.video_model.precision):
                for layer_idx in range(self.config.num_layers):
                    # === Use cached Video K/V (per-layer correct K/V) ===
                    video_k = video_kv_cache[layer_idx]["k"]
                    video_v = video_kv_cache[layer_idx]["v"]

                    # === Update VLM tokens: base features + accumulated updates ===
                    ...
                    # === Compute Action Q/K/V ===
                    action_block = self.action_expert.blocks[layer_idx]
                    a_mod = self.action_module.compute_adaln_modulation(action_adaln_params, layer_idx)
                    norm_action = action_block.norm1(action_tokens) * (1 + a_mod[1].squeeze(2)) + a_mod[0].squeeze(2)

                    a_qkv = torch.einsum("BTD,KNDE->KBTNE", norm_action, action_block.wan_action_qkv)
                    a_q = action_block.wan_action_norm_q(a_qkv[0].flatten(-2)).view(B, L_a, n, d)
                    a_k = action_block.wan_action_norm_k(a_qkv[1].flatten(-2)).view(B, L_a, n, d)
                    a_v = a_qkv[2].view(B, L_a, n, d)

                    # === Compute VLM Q/K/V (real-time, since vlm_tokens changes) ===
                    vlm_block = self.qwen3_module.blocks[layer_idx]
                    norm_vlm = vlm_block.norm1(vlm_tokens)
                    L_vlm = vlm_tokens.shape[1]
                    vlm_qkv = torch.einsum("BTD,KNDE->KBTNE", norm_vlm, vlm_block.wan_vlm_qkv)
                    vlm_q = vlm_block.wan_vlm_norm_q(vlm_qkv[0].flatten(-2)).view(B, L_vlm, n, d)
                    vlm_k = vlm_block.wan_vlm_norm_k(vlm_qkv[1].flatten(-2)).view(B, L_vlm, n, d)
                    vlm_v = vlm_qkv[2].view(B, L_vlm, n, d)

                    # === Build attention mask (same as process_joint_attention) ===
                    # Video: first_frame_causal (only 1 frame here, so no future frame masking needed)
                    # Action: sees video + action + vlm
                    # VLM: self-attention only
                    L_total = L_v + L_a + L_vlm
                    attn_mask = torch.ones((B, L_total, L_total), dtype=torch.bool, device=self.device)

                    # Video internal: first_frame_causal
                    attn_mask[:, :first_frame_tokens, first_frame_tokens:L_v] = False
                    # Video doesn't see Action/VLM
                    attn_mask[:, :L_v, L_v:] = False
                    # Action only sees Video first frame
                    attn_mask[:, L_v:L_v+L_a, first_frame_tokens:L_v] = False
                    # VLM only sees VLM self-attention
                    attn_mask[:, L_v+L_a:, :L_v+L_a] = False

                    # === Joint attention: same as wan_layer.self_attn ===
                    # But use cached Video K/V instead of computing Video Q/K/V
                    # We need a dummy video_q for the concat (video still participates as query)
                    # Use zeros - video output will be discarded anyway
                    # Actually, video must participate as query for correct attention weights
                    # Compute video Q from the current layer's cached state
                    # The video_tokens used in prefill are the correct ones for this layer
                    video_q_cache = video_kv_cache[layer_idx].get("q")
                    if video_q_cache is not None:
                        video_q = video_q_cache
                    else:
                        # Fallback: compute video Q (should not happen if cache is complete)
                        video_q = torch.zeros(B, L_v, n, d, device=self.device, dtype=self.dtype)

                    q_cat = torch.cat([video_q, a_q, vlm_q], dim=1)
                    k_cat = torch.cat([video_k, a_k, vlm_k], dim=1)
                    v_cat = torch.cat([video_v, a_v, vlm_v], dim=1)

                    seq_lens = torch.full((B,), L_total, dtype=torch.long, device=self.device)
                    ...
                    # Split outputs (same as wan_layer.self_attn)
                    video_out = attn_out[:, :L_v, :, :]
                    action_out_h = attn_out[:, L_v:L_v+L_a, :, :]
                    vlm_out_h = attn_out[:, L_v+L_a:, :, :]

                    # Project back and residual connections (same as process_joint_attention)
                    # Video output: project but discard (we don't update video_tokens)
                    # video_out_proj = wan_layer.self_attn.o(video_out.flatten(2))

                    action_out = action_block.wan_action_o(action_out_h.flatten(2))
                    vlm_out = vlm_block.wan_vlm_o(vlm_out_h.flatten(2))

                    # Only update action_tokens and vlm_tokens
                    action_tokens = action_tokens + action_out * a_mod[2].squeeze(2)
                    vlm_tokens = vlm_tokens + vlm_out

                    # Action FFN
                    action_tokens = self.action_module.process_ffn(action_tokens, a_mod, layer_idx)

                    # VLM FFN
                    vlm_tokens = self.qwen3_module.process_ffn(vlm_tokens, layer_idx)

                # Action head (extract velocity same way as inference_step)
                action_pred_full = self.action_expert.decoder(action_tokens, action_head_time_emb)
                # Extract middle action chunk (skip first state token and last register tokens)
                # action_pred_full shape: [B, 1(state) + action_chunk_size + num_registers, action_dim]
                # We want: [B, action_chunk_size, action_dim]
                action_velocity = action_pred_full[:, 1:1+action_chunk_size, :]

            # Euler integration
            action_latent = action_latent + action_velocity * dt

        return action_latent[:, :action_horizon, :].float()  # [B, action_horizon, action_dim]
