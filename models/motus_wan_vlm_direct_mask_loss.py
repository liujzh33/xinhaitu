...
from transformers import Qwen3VLForConditionalGeneration, AutoConfig, AutoTokenizer
from .wan_model_mask import WanVideoModel
from .action_expert import ActionExpert, ActionExpertConfig
from .qwen3_module_wan_loss import Qwen3VLWanModule, Qwen3VLWanConfig
from .fast_tokenizer import FastActionTokenizer
...
class MotusWanVlmDirectMaskConfig:
    ...
    vlm_action_loss_weight: float = 0.1  # VLM action loss weight (discrete tokens)
    enable_vlm_action_loss: bool = True  # Enable/disable VLM action loss
    fast_tokenizer_path: str = "physical-intelligence/fast"  # Path to fast tokenizer (local or HF Hub)
...
class MotusWanVlmDirectMask(nn.Module):
    ...
    def __init__(self, config: MotusWanVlmDirectMaskConfig):
        ...
        # Load VLM tokenizer for action token detection
        logger.info(f"Loading VLM tokenizer from {config.vlm_checkpoint_path}...")
        ...
        qwen3_config = Qwen3VLWanConfig(
            ...
            enable_vlm_action_loss=config.enable_vlm_action_loss,
        )

        self.qwen3_module = Qwen3VLWanModule(self.vlm_model, qwen3_config, self.dtype, self.device, tokenizer=self.vlm_tokenizer)

        # Initialize Fast Action Tokenizer for VLM action loss
        if config.enable_vlm_action_loss:
            logger.info(f"Initializing Fast Action Tokenizer from {config.fast_tokenizer_path}...")
            self.fast_tokenizer = FastActionTokenizer(fast_tokenizer_name=config.fast_tokenizer_path)
        else:
    ...
    def training_step(
        ...
        # First, prepare VLM inputs with action tokens for VLM action loss
        if (self.config.enable_vlm_action_loss and
            self.qwen3_module.config.action_token_min >= 0 and
            self.fast_tokenizer is not None and
            self.qwen3_module.config.enable_vlm_action_loss):

            # Convert list format to dict format if needed
            if isinstance(vlm_inputs, list):
                # Need to batch the list format first
                ...
                attention_mask_batch = torch.cat(padded_attention_masks, dim=0)
                ...
                vlm_inputs_dict = vlm_inputs

            # Prepare VLM inputs with action tokens
            vlm_inputs_with_action = self.qwen3_module.prepare_vlm_inputs_with_action(
                vlm_inputs_dict, actions, self.fast_tokenizer
            )
        else:
            vlm_inputs_with_action = vlm_inputs

        t_vlm_start = time.time()
        vlm_per_layer, vlm_action_loss = self.qwen3_module.extract_per_layer_features(vlm_inputs_with_action)
        ...
        # VLM action loss (if available)
        if vlm_action_loss is not None:
            if torch.isnan(vlm_action_loss):
                logger.warning("VLM action loss is NaN, setting to 0")
                vlm_action_loss = torch.tensor(0.0, device=self.device)
        else:
            vlm_action_loss = torch.tensor(0.0, device=self.device)
        ...
        # Add VLM action loss if enabled and available
        if self.config.enable_vlm_action_loss and vlm_action_loss.item() > 0:
            total_loss = total_loss + self.config.vlm_action_loss_weight * vlm_action_loss
        ...
        if return_dict:
            return {
                ...
                'vlm_action_loss': vlm_action_loss,
    ...
    def inference_step(
        ...
        vlm_per_layer, _ = self.qwen3_module.extract_per_layer_features(vlm_inputs)
        assert len(vlm_per_layer) == 30, f"Expected 30 VLM layers, got {len(vlm_per_layer)}"
