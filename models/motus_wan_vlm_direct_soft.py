...
# Import qwen3_module_wan_soft with learnable alignment
from .qwen3_module_wan_soft import Qwen3VLWanModule, Qwen3VLWanConfig
...
class MotusWanVlmDirectConfig:
    ...
    # Learnable Layer Alignment settings (Qwen3-VL -> WAN)
    use_learnable_alignment: bool = False  # Whether to use learnable soft alignment for VLM->WAN mapping
    ...
    alignment_init_identity: bool = False  # Initialize near-identity mapping
...
class MotusWanVlmDirect(nn.Module):
    ...
    def __init__(self, config: MotusWanVlmDirectConfig):
        ...
        qwen3_config = Qwen3VLWanConfig(
            ...
            use_learnable_alignment=config.use_learnable_alignment,
            alignment_temperature=config.alignment_temperature,
            alignment_temperature_decay=config.alignment_temperature_decay,
            alignment_init_identity=config.alignment_init_identity,
        )
    ...
    # ===== Learnable Alignment Utility Methods =====

    def update_alignment_temperature(self, progress: float, min_temp: float = 0.1):
        """
        Update the alignment temperature based on training progress.

        This should be called during training to implement temperature decay.
        ...
        Example:
            for epoch in range(num_epochs):
                ...
                model.update_alignment_temperature(progress)
                # ... training loop ...
        """
        self.qwen3_module.update_alignment_temperature(progress, min_temp)

    def get_alignment_weights(self) -> Optional[torch.Tensor]:
        """
        Get the current learnable alignment weights.

        Returns:
            Alignment matrix [wan_layers, vlm_layers], or None if alignment is disabled
        """
        return self.qwen3_module.get_alignment_weights()

    def get_alignment_entropy(self) -> Optional[float]:
        """
        Get the current alignment entropy.

        Returns:
            Average entropy across all WAN layers, or None if alignment is disabled
        """
        return self.qwen3_module.get_alignment_entropy()

    def log_alignment_info(self, step: int = 0):
        """
        Log alignment information for monitoring.

        Args:
            step: Current training step for logging
        """
        if not self.config.use_learnable_alignment:
            return

        weights = self.get_alignment_weights()
        entropy = self.get_alignment_entropy()

        logger.info(f"[Step {step}] Layer Alignment Info:")
        logger.info(f"  Temperature: {self.qwen3_module.alignment.current_temperature.item():.4f}")
        logger.info(f"  Average Entropy: {entropy:.4f}")

        # Log peak correspondence for each WAN layer
        if weights is not None:
            peak_layers = weights.argmax(dim=-1).cpu().tolist()
            logger.info(f"  Peak VLM layer per WAN layer: {peak_layers}")
