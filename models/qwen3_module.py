# Qwen3-VL Module for Direct MoT Participation
# Each layer has its own QKV projection to the unified head space
...
@dataclass
class Qwen3VLExpertConfig:
    """Configuration for Qwen3-VL Expert model."""
    vlm_dim: int = 2048              # Qwen3-VL-2B hidden size
    head_dim: int = 128              # Head dimension (for Cosmos 2B)
    num_heads: int = 16              # Number of heads (for Cosmos 2B)
    num_layers: int = 28             # Number of layers
    eps: float = 1e-5                # Layer norm epsilon


class Qwen3VLExpertBlock(nn.Module):
    """
    ...
    this block only provides QKV projections to the unified head space.
    ...
    def __init__(self, vlm_dim: int, head_dim: int, num_heads: int, eps: float = 1e-5):
        ...
        self.num_heads = num_heads  # 16 for Cosmos 2B
        self.head_dim = head_dim  # 128 for Cosmos 2B
        self.unified_dim = num_heads * head_dim  # 2048

        # LayerNorm (same style as WAN)
        self.norm1 = WanLayerNorm(vlm_dim, eps=eps)

        # QKV projection: 2048D -> Unified head space (16 heads × 128 = 2048)
        self.unified_vlm_qkv = nn.Parameter(
            torch.randn(3, num_heads, vlm_dim, head_dim)
            / (vlm_dim * head_dim) ** 0.5
        )

        # Output projection: Unified dim (2048) -> VLM dim (2048)
        # Since dimensions match, this is an identity-like projection for learning
        self.unified_vlm_o = nn.Linear(self.unified_dim, vlm_dim, bias=False)

        # Q/K normalization (RMSNorm like WAN)
        self.unified_vlm_norm_q = WanRMSNorm(self.unified_dim, eps=eps)
        self.unified_vlm_norm_k = WanRMSNorm(self.unified_dim, eps=eps)


class Qwen3VLModule(nn.Module):
    """
    Qwen3-VL Module - extracts per-layer hidden states for direct MoT.

    This module:
    1. Extracts all layer outputs from Qwen3-VL (not just last layer)
    2. Provides per-layer QKV projections to unified head space
    3. Processes residual connections after MoT attention
    """

    def __init__(self, vlm_model, config: Qwen3VLExpertConfig, dtype, device):
        super().__init__()
        self.config = config
        self.dtype = dtype
        self.device = device
        ...
        # Create per-layer projection blocks
        ...
        logger.info(f"Qwen3VLModule initialized with {config.num_layers} layers")
        logger.info(f"  VLM dim: {config.vlm_dim}, Unified head space: {config.head_dim * config.num_heads}")
    ...
    def extract_per_layer_features(
        ...
        Returns:
            List of hidden states, one per layer: [layer_0, layer_1, ..., layer_27]
        ...
        # Forward through VLM (with or without grad based on training mode)
        t1 = time.time()
        ...
        t2 = time.time()
        result = list(hidden_states[1:])  # Return only layer outputs, exclude embeddings
        timing['convert_list'] = time.time() - t2

        # Log timing breakdown (only once)
        if not hasattr(self, '_logged_extract_timing'):
            logger.info(f"Qwen3VLModule.extract_per_layer_features breakdown:")
            ...
            logger.info(f"  Convert to list: {timing['convert_list']*1000:.1f}ms")
    ...
    def _process_vlm_inputs_to_tokens(
        ...
        # 1. Vision encoder is typically frozen in multimodal training
        # 2. The trainable projections are in the language model
        t2 = time.time()
        with torch.no_grad():
        ...
        if not hasattr(self, '_logged_process_timing'):
            logger.info(f"Qwen3VLModule._process_vlm_inputs_to_tokens breakdown:")
    ...
    def process_ffn(self, vlm_tokens: torch.Tensor, layer_idx: int) -> torch.Tensor:
        ...
        The residual connection is already applied in process_joint_attention():
            qwen3_tokens = qwen3_tokens + q_out
